...
module.exports.changeSubscription = async function(username, value, callback) {
    const filter = { username: username };
    const update = { subscription: value };
    callback(false, await User.findOneAndUpdate( filter, update ));
}

module.exports.deleteUser = async function(username, callback) {
    const filter = { username: username };
    callback(false, await User.deleteOne( filter ));
}
